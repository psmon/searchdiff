        ...
        if(json.response.docs){
            for(var idx=0; idx<json.response.docs.length; idx++){
                ...
                if( json.response.docs[idx].position!=null ) {
                    ...
                    if(json.response.docs[idx].region1!=null){
                        var nationName ="";
                        if(json.response.docs[idx].mcategory1.indexOf('국내숙박') > -1 )
                            nationName = "대한민국"
                        else
                            nationName = json.response.docs[idx].region1;
                        if(RegionInfoList[nationName]==null){
                            RegionInfoList[nationName] = { name:nationName, count:1 };
                        }else{
                            RegionInfoList[nationName].count++;
                        }
                    }
            ...
            if(json.grouped.mcategory1){
                for (i = 0; i < json.grouped.mcategory1.groups.length ; i++) {
                    ...
                    for(var idx=0; idx<docs.length; idx++) {
                        ...
                        if(docs[idx].region1!=null){
                            var nationName ="";
                            if(docs[idx].mcategory1.indexOf('국내숙박') > -1 )
                                nationName = "대한민국"
                            else
                                nationName = docs[idx].region1;
                            if(RegionInfoList[nationName]==null){
                                RegionInfoList[nationName] = { name:nationName, count:1 };
                            }else{
                                RegionInfoList[nationName].count++;
